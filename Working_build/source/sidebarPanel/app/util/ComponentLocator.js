class ComponentLocator {
  ...
  configureSupportedConfigs() {
    ...
    for (let supportedConfig of configs) {
      if (supportedConfig.prop.charAt(0) != '$') {
        getter =
          'get' +
          supportedConfig.prop.charAt(0).toUpperCase() +
          supportedConfig.prop.slice(1);
  ...
  isSupportedApp() {
    if (this.win.Ext) {
      ...
      if (
        ext.versions &&
        ext.versions.extjs &&
        ext.versions.extjs.major &&
        ext.versions.extjs.major >= 4
      ) {
        return true;
      }
    }
  ...
  addLocator(xtypes, locator, container, priority, owningComponent) {
    ...
    if (container && container.xtype) {
      ...
      if (
        container.getItemId &&
        container.getItemId() != null &&
        container.getItemId() != '' &&
        containerEl &&
        containerEl.id &&
        containerEl.id != container.getItemId() &&
    ...
    if (owningComponent && owningComponent.xtype) {
      ...
      if (
        owningComponent.getItemId &&
        owningComponent.getItemId() != null &&
        owningComponent.getItemId() != '' &&
        owningComponentEl &&
        owningComponentEl.id &&
        owningComponentEl.id != owningComponent.getItemId() &&
  ...
  getComponentLocators() {
    ...
    console.log(!me.isSupportedApp());
    if (!me.isSupportedApp()) {
      return {
        error:
          'The Component Locator only supports apps developed with Sencha Ext JS 4 or higher.',
      };
    } else {
      ...
      for (let supportedConfig of me.supportedConfigs) {
        ...
        if (
          ...
          (supportedConfig.prop == 'name' &&
            locatorValue.indexOf &&
            locatorValue.indexOf(element.id) == 0)
        ) {
          continue;
        } else if (
          (xtypes.indexOf('listitem') >= 0 ||
          ...
          if (record) {
            ...
            if (typeof recordId == 'string') {
              locator =
                xtype + locatorExtra + '{_record.id=="' + recordId + '"}';
            } else {
          ...
          supportedConfig.prop == 'itemId' &&
          me.isValidLocatorValue(locatorValue)
        ) {
          ...
          locator =
            xtype +
            '[' +
            supportedConfig.prop +
            '="' +
            locatorValue.replace('"', '\\\\"').replace(',', '\\\\,') +
            '"]';
      ...
      me.locators.sort(
        (locatorA, locatorB) => locatorA.priority - locatorB.priority
      );
